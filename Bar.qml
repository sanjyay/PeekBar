...
Item {
  ...
  // PeekBar configuration options from shell.json `bar:` section.
  readonly property bool revealInFullscreen: barConfig && barConfig.revealInFullscreen !== undefined ? !!barConfig.revealInFullscreen : true
  readonly property int triggerThickness: Math.max(1, Math.min(20, (barConfig && typeof barConfig.triggerThickness === "number" ? barConfig.triggerThickness : 2)))
  readonly property int revealDelay: Math.max(0, (barConfig && typeof barConfig.revealDelay === "number" ? barConfig.revealDelay : 0))
  ...
  Connections {
    ...
    function onRawEvent(event) {
      ...
      if (name === "fullscreen" || name === "workspace" || name === "changefloatingmode") {
    ...
    delegate: Component {
      Scope {
        ...
        PeekBarController {
          ...
          revealInFullscreen: root.revealInFullscreen
  ...
  component BarPanel: PanelWindow {
    ...
    exclusionMode: controller.exclusionMode
    ...
    property real activeMarginTop: controller.targetMarginTop
    property real activeMarginBottom: controller.targetMarginBottom
    property real activeMarginLeft: controller.targetMarginLeft
    property real activeMarginRight: controller.targetMarginRight

    Binding {
      target: barWindow
      property: "activeMarginTop"
      value: controller.targetMarginTop
    }
    Binding {
      target: barWindow
      property: "activeMarginBottom"
      value: controller.targetMarginBottom
    }
    Binding {
      target: barWindow
      property: "activeMarginLeft"
      value: controller.targetMarginLeft
    }
    Binding {
      target: barWindow
      property: "activeMarginRight"
      value: controller.targetMarginRight
    }

    Behavior on activeMarginTop {
      NumberAnimation {
        duration: root.animationDuration
        easing.type: Easing.OutCubic
      }
    }
    Behavior on activeMarginBottom {
      NumberAnimation {
        duration: root.animationDuration
        easing.type: Easing.OutCubic
      }
    }
    Behavior on activeMarginLeft {
      NumberAnimation {
        duration: root.animationDuration
        easing.type: Easing.OutCubic
      }
    }
    Behavior on activeMarginRight {
      NumberAnimation {
        duration: root.animationDuration
        easing.type: Easing.OutCubic
      }
    ...
    margins {
      top: barWindow.activeMarginTop
      bottom: barWindow.activeMarginBottom
      left: barWindow.activeMarginLeft
      right: barWindow.activeMarginRight
    }
    ...
    WlrLayershell.layer: controller.layer
    ...
    Loader {
      ...
      HoverHandler {
        onHoveredChanged: {
          ...
          controller.barSurfaceHovered = hovered
        }
        ...
        Component.onDestruction: {
          if (hovered) {
            ...
            controller.barSurfaceHovered = false
          }
        }
      }
    }
